...
6. **Backend Entrypoint**: The only authorized and supported entry point for the backend is `src/server.ts` via `src/app.ts`. The old root-level `server.ts` has been completely deleted.
