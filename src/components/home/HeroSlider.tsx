        ...
        const dynamicSlides = Array.isArray(dynamicRes?.data)
          ? dynamicRes.data.map((d: any) => {
              ...
              if (typeof d.title === 'string') {
                try {
                  parsedTitle = JSON.parse(d.title);
                } catch {
                  parsedTitle = { ar: d.title, en: d.title };
                }
              }

              let parsedCaption = d.slider_caption;
              if (typeof d.slider_caption === 'string' && d.slider_caption.trim().startsWith('{')) {
                try {
                  parsedCaption = JSON.parse(d.slider_caption);
                } catch {
                  parsedCaption = { ar: d.slider_caption, en: d.slider_caption };
                }
              }
              const title = parsedCaption || parsedTitle || { ar: 'عنوان التحديث', en: 'Latest Update' };

              let parsedExcerpt = d.excerpt || d.description;
              if (typeof parsedExcerpt === 'string') {
                if (parsedExcerpt.trim().startsWith('{')) {
                  try {
                    parsedExcerpt = JSON.parse(parsedExcerpt);
                  } catch {
                    parsedExcerpt = { ar: parsedExcerpt, en: parsedExcerpt };
                  }
                } else {
                  parsedExcerpt = { ar: parsedExcerpt, en: parsedExcerpt };
                }
              ...
              let btnText = d.slider_button_text;
              if (typeof d.slider_button_text === 'string' && d.slider_button_text.trim().startsWith('{')) {
                try {
                  btnText = JSON.parse(d.slider_button_text);
                } catch {
                  btnText = { ar: d.slider_button_text, en: d.slider_button_text };
                }
              }
              if (!btnText) {
                btnText = { ar: 'استكشف التفاصيل', en: 'Explore Details' };
              }
              ...
              const typeBadge = {
                ar: type === 'article' ? 'أحدث الأنباء والتقارير' : 
                    type === 'project' ? 'أبرز المبادرات والمشاريع' : 
                    type === 'course' ? 'جديد أكاديمية التدريب' : 
                    type === 'event' ? 'الفعالية القادمة' : 
                    type === 'movie' || type === 'cinema' ? 'سينما الأربعاء - أحدث العروض' : 'تحديث جديد',
                en: type === 'article' ? 'Latest Report & News' : 
                    type === 'project' ? 'Featured Project' : 
                    type === 'course' ? 'Academy Course' : 
                    type === 'event' ? 'Upcoming Event' : 
                    type === 'movie' || type === 'cinema' ? 'Wednesday Cinema Screening' : 'Latest Update'
              };
              ...
              return {
                ...
                title,
                subtitle: typeBadge,
                description: parsedExcerpt || { ar: 'تابع تفاصيل هذا التحديث من خلال منصتنا المؤسسية.', en: 'Follow full update details through our platform.' },
                mediaType: 'image',
                mediaUrl: d.slider_image || d.mainImage || d.image || d.announcementImage || d.imageUrl || d.posterUrl || 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?auto=format&fit=crop&q=80&w=1920',
                animationType: 'fade',
                ...
                <div className="absolute inset-0 z-0 bg-black">
                  {slide.mediaType === 'video' && slide.mediaUrl ? (
                    ...
                    <motion.img 
                      ...
                      src={slide.mediaUrl || 'https://images.unsplash.com/photo-1504711434969-e33886168f5c?auto=format&fit=crop&q=80&w=1600'} 
